...
public class TransformerService {
	...
	public TransformerDto create(TransformerDto dto) {
		log.info("create() object '{}' with: {}", dto.getClass().getSimpleName(), dto);
	...
	public TransformerDto update(TransformerDto dto) {
		log.info("update() object '{}' with: {}", dto.getClass().getSimpleName(), dto);
	...
	public TransformerWarDto war(List<Long> ids) {
		log.info("war() object '{}' ids '{}'", TransformerWarDto.class.getSimpleName(), ids);
		...
		log.info("war() autobotSurvivors: '{}'", autobotSurvivors);
		...
		log.info("war() decepticonsSurvivors: '{}'", decepticonSurvivors);

		// #7. The team who eliminated the largest number of the opposing team is the winner.
		if (autobotWins > decepticonWins) {
			log.info(BATTLE_RESULT_TEMPLATE_LOG, autobotWins, decepticonWins, "Autobot", "won");
			...
			log.info(BATTLE_RESULT_TEMPLATE_LOG, autobotWins, decepticonWins, "Decepticon", "won");
		...
		log.info("war() object '{}' returned with: {}", transformerWarResponseDto.getClass().getSimpleName(), transformerWarResponseDto);
	...
	private IndividualBattleResultEnum individualBattle(TransformerDto autobot, TransformerDto decepticon) {
		...
		if (autobot.getName().trim().equalsIgnoreCase(OPTIMUS_PRIME)) {
			log.info(BATTLE_RESULT_TEMPLATE_LOG, autobot.getName(), decepticon.getName(), autobot.getName(), "won");
			return IndividualBattleResultEnum.AUTOBOT_WON;
		}
		if (decepticon.getName().trim().equalsIgnoreCase(PREDAKING)) {
			log.info(BATTLE_RESULT_TEMPLATE_LOG, autobot.getName(), decepticon.getName(), decepticon.getName(), "won");
		...
		if (autobot.getCourage() - decepticon.getCourage() <= -4
			...
			log.info(BATTLE_RESULT_TEMPLATE_LOG, autobot.getName(), decepticon.getName(), autobot.getName(), "ran away");
		...
		if (decepticon.getCourage() - autobot.getCourage() <= -4
			...
			log.info(BATTLE_RESULT_TEMPLATE_LOG, autobot.getName(), decepticon.getName(), decepticon.getName(), "ran away");
		...
		if (getOverral(autobot) > getOverral(decepticon)) {
			...
			log.info(BATTLE_RESULT_TEMPLATE_LOG, autobot.getName(), decepticon.getName(), "both", "were destroyed");
